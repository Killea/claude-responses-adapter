...
describe('Request Validation', () => {
    describe('validateAnthropicRequest', () => {
        ...
        it('should reject malformed tool_use blocks', () => {
            const invalidRequest = {
                model: 'claude-4-opus',
                max_tokens: 1024,
                messages: [{
            ...
            const result = validateAnthropicRequest(invalidRequest);
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.field.includes('tool_use') || e.field.endsWith('.id'))).toBe(true);
        });
        ...
        it('should reject unsupported content block types', () => {
            const invalidRequest = {
                ...
                messages: [{
                    role: 'user',
                    content: [{ type: 'document', text: 'nope' }]
                }]
            };

            const result = validateAnthropicRequest(invalidRequest);
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.message.includes('unsupported content block type'))).toBe(true);
        });

        it('should validate tools and tool_choice shape', () => {
            const invalidRequest = {
                model: 'claude-4-opus',
                max_tokens: 1024,
                tools: [{ description: 'missing name', input_schema: {} }],
            ...
            const result = validateAnthropicRequest(invalidRequest);
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.field === 'tools[0].name')).toBe(true);
            expect(result.errors.some(e => e.field === 'tool_choice.name')).toBe(true);
        });

        it('should validate system entry shape', () => {
            const invalidRequest = {
                model: 'claude-4-opus',
                max_tokens: 1024,
                system: [{ type: 'note', text: 1 }],
                messages: [{ role: 'user', content: 'Hello' }]
            };

            const result = validateAnthropicRequest(invalidRequest);
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.field === 'system[0].type')).toBe(true);
            expect(result.errors.some(e => e.field === 'system[0].text')).toBe(true);
        });
    });
